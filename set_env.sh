...
# If a local .env file exists in this project, prefer loading GEMINI_API_KEY from it
if [ -f ".env" ]; then
  # extract the first GEMINI_API_KEY line, remove surrounding quotes and CR/LF
  GEMINI_FROM_DOTENV=$(grep -E '^GEMINI_API_KEY=' .env | head -n 1 | sed 's/^GEMINI_API_KEY=//; s/^"//; s/"$//; s/\r$//')
  if [ -n "$GEMINI_FROM_DOTENV" ]; then
    export GEMINI_API_KEY=$(echo "$GEMINI_FROM_DOTENV" | tr -d '\r\n[:space:]')
    echo "Exported GEMINI_API_KEY from .env"
  fi
fi

# Source the user bashrc if present (optional)
